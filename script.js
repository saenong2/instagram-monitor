...
function renderSummary(accounts) {
  ...
  document.getElementById('topMinister').textContent =
    ministers[0] ? `${ministers[0].agency} (${numberFormat(ministers[0].followers)})` : '-';
  document.getElementById('topViceMinister').textContent =
    viceMinisters[0] ? `${viceMinisters[0].agency} (${numberFormat(viceMinisters[0].followers)})` : '-';
}
...
function renderPosts(posts) {
  ...
  postGrid.innerHTML = posts.map(post => `
    <div class="post-card">
      ${
        post.thumbnail
          ? `<img class="post-thumb" src="${post.thumbnail}" alt="${post.agency}" />`
